...
def EmailAuthenticate(request):
	...
	msg = "Welcome to OnPointCurrency.com. Your Authentication code is %s, Please click on the link below to complete your sign up! %s" % (auth_code, auth_link)
	RaySendMail(request, "Authentication", msg, app_user.user.username)
...
def ForgotPasswordView(request):
	if request.method == "POST":
		...
		try:
			...
			msg = "Set a new password for your account. Click on the link below to proceed! %s" % (new_password_link)
			RaySendMail(request, "Forgot Password", msg, app_user.user.username)
